...
- **Almost every walkthrough carries an architecture diagram** — anything multi-component does, in either architecture (in the Descent, the first destination-tour step anchors on it). Prefer a LikeC4 export: put the image path in `overview.diagram_image` (string for one image, or `{ "light": "...", "dark": "..." }` for theme-matched exports), and give a step its own `diagram` (+ `diagram_caption`) when one component cluster deserves a dedicated view. Resolve paths relative to `meta.repo_root` first, then the walkthrough JSON directory. Mermaid/source diagrams are fallback-only via `overview.diagram_mermaid`. The gate warns when no diagram is present at all.
