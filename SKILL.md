...
- **Takeaway grammar**: one outcome per line — never weld two outcomes with "and"/"alongside"/semicolons; split the step or demote the lesser outcome to a claim. Aim under ~30 words. A takeaway never restates its title: the title is the topic, the takeaway is what is now true (outcome + cause reads best: "…the boundary is a separate, starved executor — which is the entire reason there are two services"). Journey-tagged steps lead with the durable payload (what was learned, validated, or decided), not activity narration. If a thread spans non-adjacent steps, the later takeaway marks the continuation explicitly; if narrative order deviates from causal order, the order-breaking takeaway carries the reconciling clause. No takeaway line may force the reader to reconcile it against an earlier line.
- **One fact, one rung.** Within a step a fact lives at exactly one altitude: claims carry what changed, decisions carry why plus the alternatives, gotchas carry symptom plus resolution — none re-narrates another. For a bug fix, the root cause goes in the claim or the decision rationale, never both. A headline fact appears at most twice in the whole skim band: once in the overview framing, once in the step where it lands.
