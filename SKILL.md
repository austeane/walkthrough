...
- **Prose links are first-class — never post-process the HTML to add them.** The viewer hyperlinks narrative prose automatically: (1) repo-relative path tokens (`infra/live/dev`, `.github/workflows/infra.yml`, `scripts/ci/check-iam-authority.sh`, `justfile`) become GitHub `blob`/`tree` links from `meta.repo` + branch with zero authoring; (2) `meta.link_map` links bare unit/module identifiers that are not literal paths (e.g. `project-foundation` → `infra/live/dev/project-foundation`) — the value is either a string path or an object `{ "path": "<repo-relative-path>", "tooltip": "<hover text>" }` (use `"href"` for an external URL); (3) inline markdown `[label](https://…)` anywhere in prose renders as a real anchor — the general escape hatch for ANY link (Confluence, Jira, dashboards). `meta.link_mode` picks the target: `"github"` (default when `meta.repo` is a GitHub repo), `"editor"` (local `cursor://`), or `"off"` (paths/identifiers plain; explicit markdown links still render). Links never wrap inside code/diffs/commands/glossary terms, and never nest. If you find yourself wanting a link, add it via the JSON + re-render — do not hand-edit `walkthrough.html`. See schema → Prose links.
- **Prose links can carry hover tooltips — and inherit glossary ones for free.** A built link gets a hover tooltip (reusing the glossary tooltip engine, so it flips/clamps the same way) by precedence: (1) an explicit per-link tooltip — a markdown title `[label](url "hover text")` or a `link_map` object's `tooltip`; else (2) inheritance from the glossary, when the link's visible text matches a glossary `term`/alias (case-insensitive) OR its resolved repo-relative target equals a glossary entry's `file`. So you don't re-write a `Terragrunt` or `infra/modules/audit-logging` tooltip you already have in the glossary — link to it and it inherits. Clicking still navigates; only hover/focus shows the tooltip. Plain links with no explicit tooltip and no glossary match stay untooltip'd. See schema → Prose links → Hover tooltips.
- **Viewer chrome features are optional via `meta.ui` — all default ON.** Set `meta.ui.view_switcher: false` to drop the End State / Journey segmented control and lock the document to the End State view; `meta.ui.present_mode: false` to drop the Present toggle and the slideshow deck (the page stays in reading mode); `meta.ui.stats: false` to drop the stat strip (steps/files/commands/decisions/fixes/min read) in both reading and present views; `meta.ui.confidence_legend: false` to drop the grounded/inferred/speculative legend even when non-grounded claims exist. The flags are independent; omit `meta.ui` (or a key) and the feature stays on, so existing walkthroughs render unchanged. Use these only when a reader frame genuinely doesn't want a mode/affordance (e.g. a me-refresh that never journeys, a doc never presented, or a short tour where the stat strip is noise). See schema → Optional UI features.
