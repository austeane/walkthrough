...
`fonts.googleapis.com` / `cdn.jsdelivr.net`; overview diagram renders from inline
SVG; evidence scent label emits as `>{{ step._evidence_summary }}<` with no
adjacent whitespace; all user/agent text stays autoescaped; diff hunks render
from `hunk.rendered_html | safe`. The full `uv run pytest` suite must stay green.
